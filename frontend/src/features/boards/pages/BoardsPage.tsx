...
function BoardCard({
  ...
  const commitRename = async () => {
    ...
    if (!trimmed || trimmed === board.title) { setDraftTitle(board.title); return; }
    try {
      const updated = await boardService.updateMeta(board._id, { title: trimmed });
          ...
          {editingTitle ? (
            <input
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "Escape") { setEditingTitle(false); setDraftTitle(board.title); }
              }}
...
function CreateBoardModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const board = await boardService.create({ title: title.trim(), description: description.trim() || undefined });
      onCreate(board);
    } catch {
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm rounded-lg border border-slate-200 bg-white p-6 shadow-xl">
        <h2 className="mb-4 text-base font-semibold text-slate-900">New board</h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <div className="flex flex-col gap-1.5">
            <label htmlFor={`${id}-title`} className="text-sm font-medium text-slate-700">Title</label>
            <input
              id={`${id}-title`}
              ref={inputRef}
              value={title}
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor={`${id}-desc`} className="text-sm font-medium text-slate-700">
          ...
          <div className="mt-1 flex justify-end gap-2">
            ...
            >
              {loading ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" /> : "Create"}
            </button>
          </div>
        </form>
      </div>
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-base font-bold tracking-tight text-slate-900">CanvaSync</span>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-md bg-slate-900 px-3 py-1.5 text-sm font-medium text-white transition hover:bg-slate-700"
          >
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <span className="text-sm text-slate-400">{boards.length} board{boards.length !== 1 ? "s" : ""}</span>
        </div>
        ...
        {loading ? (
          ...
          /* Empty state */
          <div className="flex flex-col items-center justify-center gap-4 rounded-xl border-2 border-dashed border-slate-200 py-20 text-center">
